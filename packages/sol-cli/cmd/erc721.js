...
const mintCmd = new Command("mint")
  ...
  .action(async function (args) {
    ...
    log(
      args,
      `Minting token with id ${args.id} to ${args.wallet.address} on contract ${args.erc721Address}!`
    );
    const tx = await erc721Instance.mint(
      args.wallet.address,
      ethers.utils.hexlify(args.id),
      args.metadata
    );
...
const approveCmd = new Command("approve")
  ...
  .option(
    ...
    constants.ERC721_HANDLER_ADDRESS
  )
  .option(
    "--erc721Address <address>",
    "ERC721 contract address",
  ...
  .action(async function (args) {
    ...
    log(
      args,
      `Approving ${args.recipient} to spend token ${args.id} from ${args.wallet.address} on contract ${args.erc721Address}!`
    );
    const tx = await erc721Instance.approve(
      args.recipient,
      ethers.utils.hexlify(args.id),
      {
...
const depositCmd = new Command("deposit")
  ...
  .option(
    ...
    "Resource ID for transfer",
    constants.ERC721_RESOURCEID
  )
  .option(
    "--bridge <address>",
...
const createErc721ProposalData = (id, recipient, metadata) => {
  ...
  return (
    ...
    ethers.utils
      .hexZeroPad(
        ethers.utils.hexlify(metadata.length / 2 + (metadata.length % 2)),
        32
      )
      .substr(2) +
    metadata
...
const proposalDataHashCmd = new Command("data-hash")
  ...
  .option(
    "--handler <address>",
    "ERC721 handler address",
    constants.ERC20_HANDLER_ADDRESS
  )
  .action(async function (args) {
    console.log(args.metadata);
    const data = createErc721ProposalData(
      args.id,
      args.recipient,
      args.metadata
    );
