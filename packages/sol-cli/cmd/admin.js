...
const isRelayerCmd = new Command("is-relayer")
  ...
  .action(async function (args) {
    ...
    console.log(
      `[${args._name}] Address ${args.relayer} ${
        res ? "is" : "is not"
      } a relayer.`
    );
  });
...
const withdrawCmd = new Command("withdraw")
  ...
  .action(async function (args) {
    ...
    log(
      args,
      `Withdrawing tokens (${args.amountOrId}) in contract ${args.tokenContract} to recipient ${args.recipient}`
    );
    let tx = await bridgeInstance.adminWithdraw(
      args.handler,
      args.tokenContract,
      args.recipient,
      args.amountOrId
