...
const registerResourceCmd = new Command("register-resource")
  ...
  .action(async function (args) {
    ...
    log(
      args,
      `Registering contract ${args.targetContract} with resource ID ${args.resourceId} on handler ${args.handler}`
    );
...
const registerGenericResourceCmd = new Command("register-generic-resource")
  ...
  .option(
    ...
    constants.GENERIC_HANDLER_ADDRESS
  )
  .option(
    "--targetContract <address>",
    `Contract address to be registered`,
  ...
  .option(
    "--execute <string>",
    "Execute proposal function signature",
    EMPTY_SIG
  )
  .option(
    "--hash",
    "Treat signature inputs as function prototype strings, hash and take the first 4 bytes",
    false
  )
  .action(async function (args) {
    await setupParentArgs(args, args.parent.parent);
    ...
    log(
      args,
      `Registering generic resource ID ${args.resourceId} with contract ${args.targetContract} on handler ${args.handler}`
    );
    const tx = await bridgeInstance.adminSetGenericResource(
      args.handler,
      args.resourceId,
      args.targetContract,
      args.deposit,
...
const setBurnCmd = new Command("set-burn")
  ...
  .action(async function (args) {
    ...
    log(
      args,
      `Setting contract ${args.tokenContract} as burnable on handler ${args.handler}`
    );
...
const queryIsBurnCmd = new Command("query-is-burn")
  .description(
    "Get whether the token address is registered as mintable/burnable in a handler"
  )
  .option(
    "--handler <address>",
    "Handler contract address",
  ...
  .option(
    "--tokenContract <address>",
    "Token contract being queried if it is burnable",
    constants.ERC20_ADDRESS
  )
  .action(async function (args) {
    await setupParentArgs(args, args.parent.parent);
    ...
    const handlerInstance = new ethers.Contract(
      args.handler,
      constants.ContractABIs.HandlerHelpers.abi,
      args.wallet
    );
    const isBurn = await handlerInstance._burnList(args.tokenContract);
    log(
      args,
      `Getting if contract ${args.tokenContract} is registered as burnable on handler ${args.handler}: ${isBurn}`
    );
  });
...
const cancelProposalCmd = new Command("cancel-proposal")
  ...
  .action(async function (args) {
    ...
    log(
      args,
      `Setting proposal with chain ID ${args.chainId} and deposit nonce ${args.depositNonce} status to 'Cancelled`
    );
    const tx = await bridgeInstance.adminCancelProposal(
      args.chainId,
      args.depositNonce
    );
    await waitForTx(args.provider, tx.hash);
...
const queryProposalCmd = new Command("query-proposal")
  ...
  .option(
    "--dataHash <value>",
    "Hash of proposal metadata",
    constants.ERC20_PROPOSAL_HASH
  )
  .action(async function (args) {
    await setupParentArgs(args, args.parent.parent);
    const bridgeInstance = new ethers.Contract(
    ...
    const prop = await bridgeInstance.getProposal(
      args.chainId,
      args.depositNonce,
      args.dataHash
    );
...
const queryResourceId = new Command("query-resource")
  ...
  .action(async function (args) {
    ...
    const address = await handlerInstance._resourceIDToTokenContractAddress(
      args.resourceId
    );
    log(
      args,
      `Resource ID ${args.resourceId} is mapped to contract ${address}`
    );
  });
