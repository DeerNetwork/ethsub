...
contract(
  "Bridge - [voteProposal with relayerThreshold == 3]",
  async (accounts) => {
    ...
    beforeEach(async () => {
      await Promise.all([
        BridgeContract.new(
          destinationDomainID,
          [relayer1Address, relayer2Address, relayer3Address, relayer4Address],
          relayerThreshold,
          0,
          100
      ...
      depositDataHash = Ethers.utils.keccak256(
        DestinationERC20HandlerInstance.address + depositData.substr(2)
      );

      await Promise.all([
        DestinationERC20MintableInstance.grantRole(
    ...
    it("depositProposal should be automatically executed after the vote if proposal status is changed to Passed during the vote", async () => {
      ...
      const depositProposalAfterThirdVoteWithExecute =
        await BridgeInstance.getProposal(
          originDomainID,
          expectedDepositNonce,
          depositDataHash
        );
      ...
      assert.strictEqual(
        depositProposalAfterThirdVoteWithExecute._status,
        STATUS.Executed
      ); // Executed
    });
    ...
    it("Relayer's vote should be recorded correctly - yes vote", async () => {
      ...
      assert.equal(
        depositProposalAfterSecondVote._yesVotes,
        relayer1Bit + relayer2Bit
      );
      ...
      assert.equal(
        depositProposalAfterThirdVote._yesVotes,
        relayer1Bit + relayer2Bit + relayer3Bit
      );
      assert.strictEqual(
        depositProposalAfterThirdVote._status,
        STATUS.Executed
      ); // Executed
    });
    ...
    it("Relayer's address should be marked as voted for proposal", async () => {
      ...
      const hasVoted = await BridgeInstance._hasVotedOnProposal.call(
        Helpers.nonceAndId(expectedDepositNonce, originDomainID),
        depositDataHash,
        relayer1Address
      );
    ...
    it("Proposal cannot be executed twice", async () => {
      ...
      await TruffleAssert.reverts(
        executeProposal(relayer1Address),
