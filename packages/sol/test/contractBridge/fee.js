...
contract("Bridge - [fee]", async (accounts) => {
  ...
  beforeEach(async () => {
    await Promise.all([
      ...
      (BridgeInstance = BridgeContract.new(
        originDomainID,
        [relayer],
        0,
        0,
  ...
  it("deposit passes if valid amount supplied", async () => {
    ...
    await BridgeInstance.adminChangeFee(Ethers.utils.parseEther("0.5"), {
      from: relayer,
    });
    assert.equal(
      web3.utils.fromWei(await BridgeInstance._fee.call(), "ether"),
      "0.5"
    );
  ...
  it("distribute fees", async () => {
    await BridgeInstance.adminChangeFee(Ethers.utils.parseEther("1"), {
      from: relayer,
    });
    assert.equal(
      web3.utils.fromWei(await BridgeInstance._fee.call(), "ether"),
