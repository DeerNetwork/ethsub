...
contract("Bridge - [admin]", async (accounts) => {
  ...
  it("_relayerThreshold should be initialRelayerThreshold", async () => {
    assert.equal(
      await BridgeInstance._relayerThreshold.call(),
      initialRelayerThreshold
    );
  });
  ...
  it("_relayerThreshold should be initialRelayerThreshold", async () => {
    ...
    await TruffleAssert.passes(
      BridgeInstance.adminChangeRelayerThreshold(newRelayerThreshold)
    );
    assert.equal(
      await BridgeInstance._relayerThreshold.call(),
      newRelayerThreshold
  ...
  it("existingRelayer should not be able to be added as a relayer", async () => {
    ...
    await TruffleAssert.reverts(
      BridgeInstance.adminAddRelayer(existingRelayer)
    );
  ...
  it("nonRelayerAddr should not be able to be added as a relayer", async () => {
    ...
    await TruffleAssert.reverts(
      BridgeInstance.adminRemoveRelayer(nonRelayerAddr)
    );
  ...
  it("Bridge admin should be expectedBridgeAdmin", async () => {
    assert.isTrue(
      await BridgeInstance.hasRole(ADMIN_ROLE, expectedBridgeAdmin)
    );
  });
  ...
  it("Bridge admin should be changed to expectedBridgeAdmin", async () => {
    ...
    await TruffleAssert.passes(
      BridgeInstance.renounceAdmin(expectedBridgeAdmin2)
    );
    assert.isTrue(
      await BridgeInstance.hasRole(ADMIN_ROLE, expectedBridgeAdmin2)
    );
  });
  ...
  it("Should set a Generic Resource ID and contract address", async () => {
    const ChainAssetInstance = await ChainAssetContract.new();
    const resourceID = Helpers.createResourceID(
  ...
  it("Should not set the same fee", async () => {
    await TruffleAssert.reverts(
      BridgeInstance.adminChangeFee(0),
  ...
  it("Should withdraw funds", async () => {
    ...
    handlerBalance = await ERC20MintableInstance.balanceOf(
      ERC20HandlerInstance.address
    );
