...
contract("Bridge - [deposit - ERC721]", async (accounts) => {
  ...
  beforeEach(async () => {
    await Promise.all([
      ...
      BridgeContract.new(originDomainID, [], 0, 0, 100).then(
    ...
    await Promise.all([
      ...
      OriginERC721MintableInstance.mint(
        depositerAddress,
        originChainTokenID,
        genericBytes
      ),
  ...
  it("[sanity] test depositerAddress' balance", async () => {
    const originChainDepositerBalance =
      await OriginERC721MintableInstance.balanceOf(depositerAddress);
  ...
  it("[sanity] test OriginERC721HandlerInstance.address' allowance", async () => {
    const allowanceHolder = await OriginERC721MintableInstance.getApproved(
      originChainTokenID
    );
  ...
  it("ERC721 can be deposited with correct owner and balances", async () => {
    ...
    const originChainHandlerBalance =
      await OriginERC721MintableInstance.balanceOf(
        OriginERC721HandlerInstance.address
      );
  ...
  it("Deposit event is fired with expected value", async () => {
    ...
    let expectedMetaData = Ethers.utils.hexlify(
      Ethers.utils.toUtf8Bytes(genericBytes)
    );
