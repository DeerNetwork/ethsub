...
contract("Bridge - [deposit - ERC1155]", async (accounts) => {
  ...
  beforeEach(async () => {
    ...
    OriginERC1155HandlerInstance = await ERC1155HandlerContract.new(
    ...
    await Promise.all([
      ...
      OriginERC1155MintableInstance.mintBatch(
        depositerAddress,
        [originChainTokenID],
    ...
    await OriginERC1155MintableInstance.setApprovalForAll(
      OriginERC1155HandlerInstance.address,
      true,
  ...
  it("[sanity] test depositerAddress' balance", async () => {
    const originChainDepositerBalance =
      await OriginERC1155MintableInstance.balanceOf(
        depositerAddress,
        originChainTokenID
      );
    assert.strictEqual(
  ...
  it("[sanity] test OriginERC1155HandlerInstance.address' allowance", async () => {
    const originChainHandlerApprovedStatus =
      await OriginERC1155MintableInstance.isApprovedForAll(
        depositerAddress,
        OriginERC1155HandlerInstance.address
      );
  ...
  it("ERC1155 deposit can be made", async () => {
    await TruffleAssert.passes(
      BridgeInstance.deposit(destinationDomainID, resourceID, depositData, {
        from: depositerAddress,
      })
    );
  });
  ...
  it("_depositCounts should be increments from 0 to 1", async () => {
    await BridgeInstance.deposit(destinationDomainID, resourceID, depositData, {
      from: depositerAddress,
    });
  ...
  it("deposit requires resourceID that is mapped to a handler", async () => {
    await TruffleAssert.reverts(
      BridgeInstance.deposit(destinationDomainID, "0x0", depositData, {
        from: depositerAddress,
      }),
