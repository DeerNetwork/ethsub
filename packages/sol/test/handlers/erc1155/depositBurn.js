...
contract("ERC1155Handler - [Deposit Burn ERC1155]", async (accounts) => {
  ...
  beforeEach(async () => {
    ...
    await Promise.all([
      ...
      ERC1155MintableInstance1.mintBatch(
        depositerAddress,
        [tokenID],
        [tokenAmount],
        "0x0"
  ...
  it("depositAmount of ERC1155MintableInstance1 tokens should have been burned", async () => {
    await BridgeInstance.deposit(domainID, resourceID1, depositData, {
      from: depositerAddress,
    });

    const handlerBalance = await ERC1155MintableInstance1.balanceOf(
      ERC1155HandlerInstance.address,
      tokenID
    );
    assert.strictEqual(handlerBalance.toNumber(), 0);

    const depositerBalance = await ERC1155MintableInstance1.balanceOf(
      depositerAddress,
      tokenID
    );
