...
contract("E2E ERC721 - Two EVM Chains", async (accounts) => {
  ...
  beforeEach(async () => {
    await Promise.all([
      ...
      ERC721MintableContract.new("token", "TOK", "").then(
        (instance) => (OriginERC721MintableInstance = instance)
      ),
      ERC721MintableContract.new("token", "TOK", "").then(
        (instance) => (DestinationERC721MintableInstance = instance)
      ),
    ]);
    ...
    await Promise.all([
      ERC721HandlerContract.new(OriginBridgeInstance.address).then(
        (instance) => (OriginERC721HandlerInstance = instance)
      ),
      ERC721HandlerContract.new(DestinationBridgeInstance.address).then(
        (instance) => (DestinationERC721HandlerInstance = instance)
      ),
    ]);
    ...
    originDepositProposalData = Helpers.createERC721DepositProposalData(
      tokenID,
      20,
      recipientAddress,
      32,
    ...
    destinationDepositProposalData = Helpers.createERC721DepositProposalData(
      tokenID,
      20,
      depositerAddress,
      32,
  ...
  it("[sanity] ERC721HandlerInstance.address should have an allowance for tokenID from depositerAddress", async () => {
    const allowedAddress = await OriginERC721MintableInstance.getApproved(
      tokenID
    );
  ...
  it("[sanity] DestinationERC721HandlerInstance.address should have minterRole for DestinationERC721MintableInstance", async () => {
    const isMinter = await DestinationERC721MintableInstance.hasRole(
      await DestinationERC721MintableInstance.MINTER_ROLE(),
      DestinationERC721HandlerInstance.address
    );
  ...
  it("E2E: tokenID of Origin ERC721 owned by depositAddress to Destination ERC721 owned by recipientAddress and back again", async () => {
    ...
    assert.strictEqual(
      tokenOwner,
      recipientAddress,
      "tokenID wasn't transferred from depositerAddress to recipientAddress"
    );
    ...
    assert.strictEqual(
      depositerAddress,
      tokenOwner,
      "OriginERC721MintableInstance tokenID was not transferred back to depositerAddress"
    );
  });
});
