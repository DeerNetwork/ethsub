...
contract("E2E ERC721 - Same Chain", async (accounts) => {
  ...
  beforeEach(async () => {
    ...
    proposalData = Helpers.createERC721DepositProposalData(
      tokenID,
      20,
      recipientAddress,
    ...
    depositProposalDataHash = Ethers.utils.keccak256(
      ERC721HandlerInstance.address + proposalData.substr(2)
    );
  });
  ...
  it("depositAmount of Destination ERC721 should be transferred to recipientAddress", async () => {
    ...
    const tokenOwnerAfterTransfer = await ERC721MintableInstance.ownerOf(
      tokenID
    );
