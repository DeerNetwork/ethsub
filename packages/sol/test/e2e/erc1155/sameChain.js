...
contract("E2E ERC1155 - Same Chain", async (accounts) => {
  ...
  beforeEach(async () => {
    ...
    proposalData = Helpers.createERC1155DepositProposalData(
      [tokenID],
      [depositAmount],
      recipientAddress,
      "0x00"
  ...
  it("[sanity] depositerAddress' balance should be equal to initialTokenAmount", async () => {
    const depositerBalance = await ERC1155MintableInstance.balanceOf(
      depositerAddress,
      tokenID
    );
  ...
  it("depositAmount of Destination ERC1155 should be transferred to recipientAddress", async () => {
    ...
    const handlerBalance = await ERC1155MintableInstance.balanceOf(
      ERC1155HandlerInstance.address,
      tokenID
    );
    ...
    const recipientBalance = await ERC1155MintableInstance.balanceOf(
      recipientAddress,
      tokenID
    );
  ...
  it("Handler's withdraw function can be called by only bridge", async () => {
    let withdrawData;
    withdrawData = Helpers.createERC1155WithdrawData(
      ERC1155MintableInstance.address,
      depositerAddress,
      [tokenID],
      [depositAmount],
  ...
  it("Should withdraw funds", async () => {
    ...
    await ERC1155MintableInstance.safeTransferFrom(
      depositerAddress,
      ERC1155HandlerInstance.address,
      tokenID,
      depositAmount,
    ...
    handlerBalance = await ERC1155MintableInstance.balanceOf(
      ERC1155HandlerInstance.address,
      tokenID
    );
