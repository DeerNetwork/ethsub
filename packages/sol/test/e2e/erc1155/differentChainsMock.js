...
contract("E2E ERC1155 - Two EVM Chains", async (accounts) => {
  ...
  beforeEach(async () => {
    await Promise.all([
      BridgeContract.new(
        originDomainID,
        [originRelayer1Address, originRelayer2Address],
        originRelayerThreshold,
        0,
        100
      ).then((instance) => (OriginBridgeInstance = instance)),
      BridgeContract.new(
        destinationDomainID,
        [destinationRelayer1Address, destinationRelayer2Address],
        destinationRelayerThreshold,
        0,
        100
      ).then((instance) => (DestinationBridgeInstance = instance)),
      ERC1155MintableContract.new("TOK").then(
        (instance) => (OriginERC1155MintableInstance = instance)
      ),
      ERC1155MintableContract.new("TOK").then(
        (instance) => (DestinationERC1155MintableInstance = instance)
      ),
    ]);
    ...
    await Promise.all([
      ERC1155HandlerContract.new(OriginBridgeInstance.address).then(
        (instance) => (OriginERC1155HandlerInstance = instance)
      ),
      ERC1155HandlerContract.new(DestinationBridgeInstance.address).then(
        (instance) => (DestinationERC1155HandlerInstance = instance)
      ),
    ]);

    await OriginERC1155MintableInstance.mintBatch(
      depositerAddress,
      [tokenID],
      [initialTokenAmount],
      "0x0"
    ...
    await Promise.all([
      OriginERC1155MintableInstance.setApprovalForAll(
        OriginERC1155HandlerInstance.address,
        true,
        { from: depositerAddress }
      ),
      DestinationERC1155MintableInstance.grantRole(
      ...
      DestinationBridgeInstance.adminSetBurnable(
        DestinationERC1155HandlerInstance.address,
        destinationBurnableContractAddresses[0]
      ),
    ]);

    originDepositData = Helpers.createERC1155DepositData(
      [tokenID],
      [depositAmount]
    );
    originDepositProposalData = Helpers.createERC1155DepositProposalData(
      [tokenID],
      [depositAmount],
      recipientAddress,
      "0x00"
    ...
    destinationDepositData = Helpers.createERC1155DepositData(
      [tokenID],
      [depositAmount]
    );
    destinationDepositProposalData = Helpers.createERC1155DepositProposalData(
      [tokenID],
      [depositAmount],
      depositerAddress,
      "0x00"
  ...
  it("[sanity] depositerAddress' balance of tokenID should be equal to initialTokenAmount", async () => {
    const depositerBalance = await OriginERC1155MintableInstance.balanceOf(
      depositerAddress,
      tokenID
    );
  ...
  it("[sanity] DestinationERC1155HandlerInstance.address should have minterRole for DestinationERC1155MintableInstance", async () => {
    const isMinter = await DestinationERC1155MintableInstance.hasRole(
      await DestinationERC1155MintableInstance.MINTER_ROLE(),
      DestinationERC1155HandlerInstance.address
    );
  ...
  it("E2E: tokenID of Origin ERC1155 owned by depositAddress to Destination ERC1155 owned by recipientAddress and back again", async () => {
    ...
    depositerBalance = await OriginERC1155MintableInstance.balanceOf(
      depositerAddress,
      tokenID
    );
    assert.strictEqual(
      depositerBalance.toNumber(),
      initialTokenAmount - depositAmount
    );

    // // destinationRelayer1 creates the deposit proposal
    await TruffleAssert.passes(
      DestinationBridgeInstance.voteProposal(
        originDomainID,
    ...
    recipientBalance = await DestinationERC1155MintableInstance.balanceOf(
      recipientAddress,
      tokenID
    );
    assert.strictEqual(
    ...
    await DestinationERC1155MintableInstance.setApprovalForAll(
      DestinationERC1155HandlerInstance.address,
      true,
    ...
    // destinationRelayer1 creates the deposit proposal
    await TruffleAssert.passes(
      OriginBridgeInstance.voteProposal(
        destinationDomainID,
        expectedDepositNonce,
        originResourceID,
